<template>
    ...
    <div class="comment_theme" :id="'item_comment_'+model.id" v-else>
        <div class="comment_area">
            <div :class="{comment_area_right:!rtl ,comment_area_right_rtl:rtl }">
                <img class="commenter_avatar" :src="link">
            </div>
            <div :class="{comment_area_left:!rtl , comment_area_left_rtl:rtl}">
                <div :class="{comment_content:!rtl,comment_content_rtl:rtl}">
                    <div v-if="model.quote_id >0 ">
                        <div :class="{comment_content:!rtl,comment_content_rtl:rtl}" class="main_quote">
                ...
                <div class="clearfixed"></div>
                <div :class="{comment_btn_area:!rtl,comment_btn_area:rtl}" v-if="canComment">
                    <button @click="showForm" :data-item_id='model.id' :data-target_id="model.target_id" :data-target_type="model.target_type" :data-parent_id='model.encode_parent_id' class="btn btn-default btn-sm btn_reply_comment" type="button">
                        <i class="fa fa-reply"></i>
                        <span>{{t('reply')}}</span>
                    </button>
                    <button v-scroll-to="'#mainForm'" @click="setQuote" :data-id="model.encode_id"  ref="quoteButton"   class="btn btn-default btn-sm btn_quote_comment" type="button">
